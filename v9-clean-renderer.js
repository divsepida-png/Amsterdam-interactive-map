'use strict';
/* V9 clean renderer.
   This replaces the entire legacy V4-V8 patch chain.
   Red/blue information is represented by one compact area marker per category.
   No rectangles, repeated bricks, dashed strips, façade ticks, building polygons or Overpass geometry are created. */
(function applyCleanV9(){
  const V9={frontageAreas:[]};
  ...
  function visualSpec(item){
    if(item.kind==='red-light')return {colour:'#d71931',text:'#fff',code:'R',group:'red',area:true};
    if(item.kind==='blue-light')return {colour:'#087df0',text:'#fff',code:'B',group:'blue',area:true};
  ...
  function badgeIcon(item,spec,extra=''){
    const classes=['v9-dot',spec.major?'major':'',spec.area?'area':'',extra].filter(Boolean).join(' ');
    return L.divIcon({
      className:classes,
      html:`<span class="v9-core" style="--poi:${spec.colour};--poi-text:${spec.text}">${esc(spec.code)}</span>`,
  ...
  function frontageCentre(frontages){
    const points=frontages.map(frontage=>[
      (frontage.points[0][0]+frontage.points[1][0])/2,
      (frontage.points[0][1]+frontage.points[1][1])/2
    ]);
    return [
      points.reduce((sum,p)=>sum+p[0],0)/Math.max(points.length,1),
      points.reduce((sum,p)=>sum+p[1],0)/Math.max(points.length,1)
    ];
  }

  function addFrontages(group,frontages,colour,kind,code){
    if(!frontages?.length)return;
    const centre=frontageCentre(frontages);
    const item={
      id:`${kind}-area`,
      name:kind==='red-light'?'Red-light district reference area':'Blue-light reference area',
      kind,lat:centre[0],lon:centre[1],
      note:'Approximate area reference only. It does not mark individual windows, exact premises or current activity.'
    };
    const spec={colour,text:'#fff',code,group:kind==='red-light'?'red':'blue',area:true};
    V9.frontageAreas.push(item);
    addSemanticPlace(group,item,spec);
  }

  const previousCategory=category;
  category=function(item){
    if(item.kind==='red-light')return 'Approximate red-light district reference area';
  ...
  allPlaces=()=>[
    ...
    ...DATA.landmarks,...DATA.transit.stops,...DATA.bars,...DATA.shops,...V9.frontageAreas
  ];

  buildLayers=function(){
    addFrontages(makeGroup('red'),DATA.redFrontages,'#d71931','red-light','R');
  ...
  style.id='v9-clean-renderer-style';
  style.textContent=`
  .v9-dot{background:transparent!important;border:0!important;overflow:visible!important}
  .v9-dot .v9-core{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);display:grid;place-items:center;width:18px;min-width:18px;height:18px;padding:0 2px;border-radius:50%;background:var(--poi);color:var(--poi-text);border:2px solid #fff;font-size:6.8px;line-height:1;font-weight:1000;box-shadow:0 0 0 1px rgba(0,0,0,.55),0 2px 5px rgba(0,0,0,.35)}
  .v9-dot.major .v9-core{width:28px;min-width:28px;height:28px;font-size:8px;border-width:3px;box-shadow:0 0 0 2px rgba(255,212,0,.85),0 3px 8px rgba(0,0,0,.42)}
  .v9-dot.area .v9-core{width:24px;min-width:24px;height:24px;font-size:10px;border-width:3px;box-shadow:0 0 0 2px rgba(0,0,0,.65),0 3px 8px rgba(0,0,0,.4)}
  ...
  if(wrap&&!document.getElementById('mapKey'))wrap.insertAdjacentHTML('beforeend',`<div id="mapKey" aria-label="Map category key">
    <span class="mapKeyChip" style="--chip:#d71931">R Area</span>
    <span class="mapKeyChip" style="--chip:#087df0">B Area</span>
  ...
  if(layers)layers.innerHTML=`<h2>Map layers</h2>
    <div class="card"><strong>Clean V9 display</strong><small>Red and blue categories use one approximate area marker each. There are no window bricks, strips, façade lines or building fills.</small></div>
    <button class="toggle on" data-layer="red"><span>Red-light district reference area — R</span><b>✓</b></button>
    <button class="toggle on" data-layer="blue"><span>Blue-light reference area — B</span><b>✓</b></button>
    ...
    <div class="card"><strong>Area accuracy</strong><small>The R and B markers indicate broad reference areas only. They do not identify individual windows, exact premises or current activity.</small></div>`;

  const locateCard=document.querySelector('#panel-locate .card');
  if(locateCard)locateCard.innerHTML='<strong>Clean marker mode</strong><small>All places use compact markers. Red and blue information has been reset to one broad area marker per category, with no bricks or frontage geometry.</small>';
})();